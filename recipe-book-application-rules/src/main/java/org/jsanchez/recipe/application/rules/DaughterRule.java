 ...
 * Rules for 'DAUGHTER'
 */
public class DaughterRule extends PrepareTimeUtil implements Rule<InquiryRequest, Recipe> {
    ...
    public InquiryResponse buildResponse(Recipe recipe) {
        ...
        if (comparatorResult < 0) {
            inquiryResponse.setPrepareTime(subtract(recipe.getPrepareTime(), minutesToHours(15.00)));
        } else if (comparatorResult > 0) {
            inquiryResponse.setPortions(null);
            inquiryResponse.setWorth(WorthType.MEH);
        } else {
            inquiryResponse.setPrepareTime(subtract(recipe.getPrepareTime(), minutesToHours(10.00)));
        }
